    ...
    public function makeData($request, $option_equipments, $option_other_names ){
        ...
        foreach($option_equipments as $key => $equipments){
            if($equipments->unit === NULL){
                $ary[$key] = isset($request['option_equipments'][$key])? $request['option_equipments'][$key]: 0;
            }else{
                $ary[$key] = isset($request['option_equipments'][$key])? $request['option_equipments'][$key]: '';
            }
        }
        ...
        foreach($option_other_names as $key => $name){
            $ary[$key] = isset($request['option_others'][$key])? $request['option_others'][$key]: 0;
        }
